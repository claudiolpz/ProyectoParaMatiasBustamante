...
export const getSellButtonStyle = (product: Product, sellLoading: boolean): string => {
  ...
  return 'text-orange-400 hover:bg-slate-600 hover:text-orange-300 border-slate-500 hover:border-orange-400';
};
...
export const getImageButtonStyle = (product: Product): string => {
  return `w-16 h-16 sm:w-20 sm:h-20 rounded-lg border transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-700 p-0 ${
    product.isActive
...
export const getImagePlaceholderStyle = (product: Product): string => {
  return `w-16 h-16 sm:w-20 sm:h-20 bg-slate-600 rounded-lg flex items-center justify-center border ${
    product.isActive ? 'border-slate-500' : 'border-slate-600 opacity-75'
  }`;
};