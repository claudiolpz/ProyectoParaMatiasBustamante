...
const SalesTable = () => {
  const { sales, loading, pagination, fetchSales } = useSalesTable();
  ...
  const handleSort = useCallback((field: 'createdAt' | 'totalPrice' | 'quantity' | 'unitPrice') => {
    ...
    setFilters(newFilters);
    fetchSales(newFilters, pagination.current);
  }, [filters, pagination.current, fetchSales]);
  ...
  const handleRefresh = useCallback(() => {
    fetchSales(filters, pagination.current);
    fetchCategories();
    fetchUsers();
  }, [fetchSales, fetchCategories, fetchUsers, pagination.current, filters]);
